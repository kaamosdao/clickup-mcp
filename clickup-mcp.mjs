...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description:
        "Получить сгруппированный отчёт по time entries. По умолчанию группировка по проекту (folder). Показывает итоговое время, полные (x1.25) и детали по каждой записи. Итоговое время — это \"чистые часы\" (реальные из ClickUp), а \"полные (x1.25)\" — это \"грязные часы\" (чистые * 1.25). ВАЖНО: возвращённый текст отчёта нужно показывать пользователю ЦЕЛИКОМ и ДОСЛОВНО — со всеми полями каждой записи (интервал начало→конец, длительность, название задачи, борда в [скобках], пользователь, billable/unbillable). НЕ сокращай, НЕ опускай строки и поля, не переформатируй на своё усмотрение.",
      inputSchema: {
        type: "object",
        properties: {
          date: {
